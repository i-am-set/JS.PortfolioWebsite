    ...
    return `
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ${projects.map(project => `
                <div class="bg-background/50 border border-secondary/30 rounded-2xl overflow-hidden flex flex-col">
                    <div class="h-48 w-full overflow-hidden bg-surface relative shrink-0">
                        <img src="${project.imageUrl}" alt="${project.title}" class="w-full h-full object-cover" onerror="this.style.display='none'">
                    </div>
                    <div class="p-6 flex flex-col flex-grow">
                        ...
                        <div class="flex gap-4 mt-auto pt-4 border-t border-secondary/30">
                            ${project.repoUrl ? `<a href="${project.repoUrl}" target="_blank" class="text-sm font-medium text-primary hover:text-accent transition-colors">GitHub &rarr;</a>` : ''}
                            ${project.liveUrl ? `<a href="${project.liveUrl}" target="_blank" class="text-sm font-medium text-primary hover:text-accent transition-colors">Live Demo &rarr;</a>` : ''}
                        </div>
                    </div>
                </div>
            `).join('')}
